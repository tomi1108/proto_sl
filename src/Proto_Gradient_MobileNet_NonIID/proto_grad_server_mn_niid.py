...
with open('./../../results/proto_grad_mn_niid/loss.txt', 'w') as f:
    f.write("")

with open('./../../results/proto_grad_mn_niid/accuracy.txt', 'w') as f:
    f.write("")

with open('./../../results/proto_grad_mn_niid/proto_loss.txt', 'w') as f:
    f.write("")
...
for round in range(cfg['param']['rounds']):
    ...
    for epoch in range(cfg['param']['epochs']):
        ...
        for iter in tqdm(range(num_iterations['client1'])):
            ...
            for connection in connection_list:
                ...
                with torch.no_grad():
                    input_features = proto_linear(smashed_data.clone().detach())
            ...
            if iter % 100 == 0:
                ...
                with open('./../../results/proto_grad_mn_niid/loss.txt', 'a') as f:
                    f.write("Iteration: {}, Loss: {:.4f}\n".format(cur_iter, agerage_loss))
                
                if use_proto:
                    print("Proto Loss: {:.4f}".format(proto_loss.item()))
                    with open('./../../results/proto_grad_mn_niid/proto_loss.txt', 'a') as f:
    ...
    with open('./../../results/proto_grad_mn_niid/accuracy.txt', 'a') as f:
