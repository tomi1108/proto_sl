...
train_data_list = [ [] for _ in range(num_classes) ]
for idx, (data, target) in enumerate(train_dataset):
    train_data_list[target].append(idx)

subset_indices = []
for i in range(len(train_data_list)):
    len_data = len(train_data_list[i]) // cfg['param']['num_client']
    split_indices, _ = random_split(train_data_list[i], [ len_data, len(train_data_list[i]) - len_data ])
    subset_indices.extend(split_indices)

train_subset = Subset(train_dataset, subset_indices)
train_loader = DataLoader(dataset=train_subset, batch_size=cfg['param']['batch_size'], shuffle=True)
...
test_num_iteraions = len(test_loader)

iteration_dict = {'num_iterations': num_iterations, 'test_num_iterations': test_num_iteraions}
